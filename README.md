...
Поддерживает режим LRS (выше дальность) и Sport (выше частота обмена данным) в модуляции FSK. Экспериментально добавлен режим Ultra Long (ulrs) на модуляции LORA с параметрами 125кгц/7sf/5cr, имеющий самые большие задержки, но в теории обеспечивающий самую высокую дальность. Нужно тестирование этого режима.
...
Тайминг передачи каждого фрейма для LRS: 70мс без rx_silent и 32мс при rx_silent, для Sport режима 40 и 22мс. Для ULRS это предварительно 123мс / 63мс.
