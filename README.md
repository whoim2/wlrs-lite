...
### Общая информация
Система радиоуправления хоббийного направления, поддерживающая SBUS и MAVLINK v1, также Frsky S.Port на TX (требует тестов и отладки, пока отключен до появления желающих). С версии 009 присутствует выбор между SBUS и CRSF для совместимости с elrs/tbs, на 420к baud.
Построена на esp32s2 (например плата lolin s2 mini) и радиомодуле с чипом sx1278/1276 (например, e32-400m20s / e32-900m20s).
Диапазоны частот: 433 и 868, требуются соответствующие модули E32. В первую очередь система позиционируется как дальнобойная для самолетов либо других не-спортивных БПЛА, где наличие небольших задержек некритично, но важна максимальная дальность. Для прочих - существует elrs, которая подойдёт больше.
...
Внимание! В скоростных режимах радиомодули нагреваются при передаче, проверяйте температуру! При необходимости установите радиаторы и даже кулер на TX. Максимальное значение мощности для 30s модулей (1вт) - число 17. Для 20s (0.1вт) - 20. Минимум - 2. Уровни мощности устанавливаются раздельно для TX и RX.
...
TXEN_PIN  39
GND       GND
VCC       VBUS //для 1вт модуля
//для 0.1 вт требуется линейный понижающий стабилизатор до 3.3в!
```
...
Для входа в режим конфигуратора на TX подайте питание и в течение секунды нажмите и удерживайте кнопку BOOT, как загорится светодиод - отпустите. Если не отпустить в течение 1,5 сек, произойдет сброс настроек до заводских и перезагрузка устройства. Это может быть иногда необходимо при изменениях в новых версиях, затрагивающих хранилище конфигурации.
...
К одному TX можно подключать неограниченное число RX, но конечно, использовать их одновременно нельзя. Хотя в rx silent режиме они все будут получать данные с TX.

### Дополнительные выходы S1-S4
На GPIO 09, 08, 07, 06 добавлены дополнительные выходы S1-S4, которые можно настроить на нужный канал и использовать как servo/gpio без участия полетника.
...
Модуль RX вводится в OTA так: после подачи питания зажимаем кнопку, держим около 2,5 секунд, в процессе светодиод загорается, затем коротко моргнет и продолжит гореть, после чего кнопку можно отпустить. Также при этом очищается конфигурация, потребуется новый бинд.
